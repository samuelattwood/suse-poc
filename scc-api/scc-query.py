...
def print_package_info(url_base, pkg_ele):
    ...
    print(f"License: {lic.text}")
    print(f"Arch: {arch.text}")
    print(f"Version: {version.attrib['ver']}-{version.attrib['rel']}")
    print(f"Checksum: {checksum.text}")
    print(f"URL: {url_base[0]}{location.attrib['href']}{url_base[1]}\n")
...
def parse_xml_gzip(h, url_base, ele):
    ...
    url = f"{url_base[0]}{href}{url_base[1]}"
    ...
    for content in output:
        ...
        if 'url' in content:
            ...
            if regex.match(curr):
                ...
                url_base = (base, token)
                #Fetch repodata
                url = f"{url_base[0]}repodata/repomd.xml{url_base[1]}"
                ...
                for ele in elems:
                    ...
                    for pkg in xml.findall("./package[@type='rpm']"):
                        print(f"Product: {content['name']}")
                        print(f"Product Description: {content['description']}")
                        if 'distro_target' in content:
                            print(f"Distro Target: {content['distro_target']}")
